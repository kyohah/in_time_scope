...
Cet exemple montre comment implémenter un système de points avec dates d'expiration en utilisant `in_time_scope`. Les points peuvent être pré-accordés pour devenir actifs dans le futur, éliminant le besoin de tâches cron.

Voir aussi : [spec/point_system_spec.rb](../../spec/point_system_spec.rb)

## Cas d'utilisation

- Les utilisateurs gagnent des points avec des périodes de validité (date de début et d'expiration)
- Les points peuvent être pré-accordés pour s'activer dans le futur (ex : bonus mensuels d'adhésion)
- Calculer les points valides à n'importe quel moment sans tâches cron
- Interroger les points à venir, expirés, etc.

## Plus besoin de tâches Cron

**C'est la fonctionnalité killer.** Les systèmes de points traditionnels sont un cauchemar de tâches planifiées :

### L'enfer Cron habituel
...
# - Redis (pour Sidekiq)
# - Cron ou gem whenever
# - Monitoring des échecs de tâches
...
user.points.in_time(1.year.ago)        # Étaient valides l'an dernier (audit !)
...
| Redis/base de données pour jobs | Requis | **Non nécessaire** |
| Planificateur de jobs (cron) | Requis | **Non nécessaire** |
| Colonne status | Requis | **Non nécessaire** |
| Migration pour mettre à jour status | Requis | **Non nécessaire** |
...
Avec les systèmes basés sur cron, répondre à "Combien de points avait l'utilisateur X le 15 janvier ?" nécessite un logging d'audit complexe ou de l'event sourcing.
...
# Points pré-planifiés pour membres de 6 mois
...
### Interroger les points

```ruby
# Points valides actuels
...
## Flux automatique de bonus d'adhésion

Pour les membres premium de 6 mois, vous pouvez configurer des bonus récurrents **sans cron, sans Sidekiq, sans Redis, sans monitoring** :
...
- **Pas de mises à jour perdues** : Les échecs de jobs peuvent laisser des points dans de mauvais états. InTimeScope n'a pas d'état à corrompre.

### Simplicité

- **Pas d'infrastructure** : Supprimez Sidekiq. Supprimez Redis. Supprimez le monitoring de jobs.
- **Pas de migrations pour changements de status** : Le temps EST l'état. Pas besoin de requêtes `UPDATE`.
- **Pas de débogage de logs de jobs** : Interrogez simplement la base de données pour voir exactement ce qui se passe.
...
| Tests | Voyage dans le temps + exécution de jobs | **Juste des requêtes** |
| Modes de défaillance | Nombreux (échecs de jobs, concurrence) | **Aucun** |
...
3. **Utilisez `in_time(time)` pour les audits** afin de vérifier les soldes de points à n'importe quel moment historique.

4. **Combinez avec les scopes inverses** pour construire des tableaux de bord admin affichant les points en attente/expirés.
